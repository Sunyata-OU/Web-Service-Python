...
from src.exceptions import ValidationError as AppValidationError, create_error_response
...
class FileValidator:
    ...
    def validate_file_upload(
        ...
        if filename and '.' in filename:
            ...
            if allowed_types:
                allowed_extensions = [
                    mime_type.split('/')[-1] for mime_type in allowed_types
                    if '/' in mime_type
                ]
                if allowed_extensions and extension not in allowed_extensions:
                    errors['filename'] = f"File extension not allowed. Allowed extensions: {', '.join(allowed_extensions)}"
