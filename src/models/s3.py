...
class S3Object(AsyncBaseModel):
    ...
    async def get_by_bucket_and_object(
        cls, db: AsyncSession, bucket_name: str, object_name: str
    ) -> Optional["S3Object"]:
        """Get S3Object by bucket name and object name."""
        from sqlalchemy import select

        result = await db.execute(select(cls).where(cls.bucket_name == bucket_name, cls.object_name == object_name))
        return result.scalar_one_or_none()

    @classmethod
    async def get_by_file_type(cls, db: AsyncSession, file_type: str, limit: int = 100) -> list["S3Object"]:
        """Get S3Objects by file type."""
        return await cls.get_multi(db, limit=limit, filters={"file_type": file_type})
