...
from sqlalchemy import Column, DateTime, Integer, select, func, update, delete
...
class AsyncBaseModel(Base):
    ...
    async def exists(
        ...
        result = await db.execute(
            select(func.count(cls.id)).where(cls.id == id)
        )
                ...
                if hasattr(self, relation):
                    ...
                    if relation_obj:
                        if isinstance(relation_obj, list):
                            ...
                            result[relation] = (
                                relation_obj.to_dict() 
                                if hasattr(relation_obj, 'to_dict') 
                                else str(relation_obj)
                            )
